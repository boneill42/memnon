...
import com.hmsonline.virgil.exception.VirgilException;
...
public class CassandraStorage {
    ...
    public CassandraStorage(VirgilConfiguration config) throws Exception {
        try {
            cluster = Cluster.builder().addContactPoints(VirgilConfiguration.getHost()).build();
        } catch (NoHostAvailableException e) {
            throw new RuntimeException(e);
        }
    }

    public JSONArray getKeyspaces() throws VirgilException, UnsupportedEncodingException {
        List<KeyspaceMetadata> keyspaces = cluster.getMetadata().getKeyspaces();
        return JsonMarshaller.marshallKeyspaces(keyspaces, true);
    }

    public void addKeyspace(String keyspace, String strategy, int replicationFactor) throws VirgilException {
        String statement = "create keyspace %s with replication = {'class':'%s', 'replication_factor':'%s'};";
        statement = String.format(statement, keyspace, strategy, replicationFactor);
        executeStatement(keyspace, statement);
    }

    public void dropTable(String keyspace, String columnFamily) throws VirgilException {
        String statement = String.format("drop keyspace %s;", keyspace);
        executeStatement(keyspace, statement);
    }

    public void dropKeyspace(String keyspace) throws VirgilException {
        String statement = String.format("drop keyspace %s;", keyspace);
        executeStatement(keyspace, statement);
    }

    // TODO: Add compactStorage flag.
    public void createTable(String keyspace, String columnFamilyName, JSONObject columns, JSONArray keys)
            throws VirgilException {
        if (columns == null || keys == null)
            throw new VirgilException("Must specify columns and keys when creating a column family.");
    ...
    public void update(String keyspace, String table, JSONObject row, JSONObject where,
            ConsistencyLevel consistency_level) throws VirgilException, HttpException, IOException {
        this.update(keyspace, table, row, where, consistency_level, System.currentTimeMillis() * 1000);
    }

    @SuppressWarnings("rawtypes")
    public void update(String keyspace, String table, JSONObject row, JSONObject where,
            ConsistencyLevel consistency_level, long timestamp) throws VirgilException, HttpException, IOException {
        if (row == null || where == null)
            throw new VirgilException("Must specify columns and values when updating a column family.");
        ...
        while (rowIterator.hasNext()) {
            ...
            Assignment assignment = QueryBuilder.set((String) pair.getKey(), pair.getValue());
        ...
        while (whereIterator.hasNext()) {
            ...
            Clause clause = QueryBuilder.eq((String) pair.getKey(), pair.getValue());
    ...
    @SuppressWarnings("rawtypes")
    public void delete(String keyspace, String table, JSONArray columns, JSONObject where,
            ConsistencyLevel consistency_level) throws VirgilException, HttpException, IOException {
    ...
    @SuppressWarnings("rawtypes")
    public JSONArray select(String keyspace, String table, JSONArray columns, JSONObject where,
            ConsistencyLevel consistencyLevel) throws VirgilException, CharacterCodingException {
    ...
    @SuppressWarnings("rawtypes")
    private StringBuilder buildList(Iterator iterator, StringBuilder sb, String delim) {
