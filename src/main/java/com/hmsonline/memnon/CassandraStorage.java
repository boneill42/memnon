...
public class CassandraStorage {
    ...
    public CassandraStorage(MemnonConfiguration config) throws Exception {
        try {
            cluster = Cluster.builder().addContactPoints(MemnonConfiguration.getHost()).build();
        } catch (NoHostAvailableException e) {
            throw new RuntimeException(e);
        }
    }

    public JSONArray getKeyspaces() throws MemnonException, UnsupportedEncodingException {
        List<KeyspaceMetadata> keyspaces = cluster.getMetadata().getKeyspaces();
        return JsonMarshaller.marshallKeyspaces(keyspaces, true);
    }

    public void addKeyspace(String keyspace, String strategy, int replicationFactor) throws MemnonException {
    ...
    public void dropTable(String keyspace, String columnFamily) throws MemnonException {
    ...
    public void createTable(String keyspace, String columnFamilyName, JSONObject columns, JSONArray keys)
            throws MemnonException {
        if (columns == null || keys == null)
            throw new MemnonException("Must specify columns and keys when creating a column family.");
    ...
    public void update(String keyspace, String table, JSONObject row, JSONObject where,
            ConsistencyLevel consistency_level) throws MemnonException, IOException {
        this.update(keyspace, table, row, where, consistency_level, System.currentTimeMillis() * 1000);
    }

    @SuppressWarnings("rawtypes")
    public void update(String keyspace, String table, JSONObject row, JSONObject where,
            ConsistencyLevel consistency_level, long timestamp) throws MemnonException, IOException {
        if (row == null || where == null)
            throw new MemnonException("Must specify columns and values when updating a column family.");
    ...
    public JSONArray select(String keyspace, String table, JSONArray columns, JSONObject where,
            ConsistencyLevel consistencyLevel) throws MemnonException, CharacterCodingException {
